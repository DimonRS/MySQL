...
SELECT id, login
FROM clients
WHERE id IN (SELECT user_id, title, full_text FROM posts WHERE title = 'Etiam ornare');

SELECT *
FROM clients, posts
WHERE cliens.id = posts.user_id = 1;

SELECT login, full_text
FROM clients, posts
WHERE title = 'Etiam ornare';

-- №3.	Выполните поиск  по публикациям, автором которых является клиент "Mikle".

SELECT login, title, user_id
FROM clients, posts
WHERE login = 'Mikle';

SELECT title, user_id
FROM posts
WHERE user_id = (SELECT id FROM clients WHERE login = 'Mikle');