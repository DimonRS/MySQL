/* 1.1 Таблицы для домашнего задания №4 дополнительное */

-- 1. Создание БД
CREATE DATABASE IF NOT EXISTS HomeWork_4_dop; -- Создаем БД lesson1, если до этого такой БД не существовло
...
FROM likes 
WHERE media_id IN (SELECT id FROM media WHERE user_id IN (SELECT user_id FROM profiles WHERE (TIMESTAMPDIFF(YEAR, birthday, NOW())< 12)));
